...
struct CounterAppState: AppState {
    ...
    static func reducer(action: Action, state: AppState) -> AppState {
        ...
        case .removeCounter(let id):
            ...
            return CounterAppState(counters: newCounters,
                                   ...
                                   totalCount: totalCountReducer(counters: newCounters))
        default: return valueChangedReducer(action, state: counterState)
        }
    }

    static func valueChangedReducer(_ action: Action, state: AppState) -> AppState {
        guard let counterState = state as? CounterAppState else {
            return state
        }
    ...
    private static func totalCountReducer(counters: [Counter]) -> Int {
        return counters.map{ $0.count }.reduce(0, +)
    }

    private static func increment(counter: Counter) -> Counter {
        return changeCount(+, counter: counter)
    }

    private static func decrement(counter: Counter) -> Counter {
        return changeCount(-, counter: counter)
    }

    private static func changeCount(_ sign: (Int, Int) -> Int, counter: Counter) -> Counter {
